...
export function resolveContentExpandedTopicLineage(
  job: ContentJob,
  ...
  if (!isRecord(topicExpansion)) {
    throw new Error("Content topic expansion metadata is invalid");
  }
  ...
  if (!isRecord(lineage)) {
    throw new Error("Content topic expansion lineage is required");
  }

  if (!isRecord(safeguards)) {
    throw new Error("Content topic expansion safeguards are required");
  }
  ...
  if (
    ...
    throw new Error("Content topic expansion safeguards are invalid");
  }
