[CmdletBinding()]
param(
    [string]$ProjectRef = "jboubdidadgfhjwdvikb",
    [string]$SupabaseUrl = "https://jboubdidadgfhjwdvikb.supabase.co",
...
$ExpectedProjectRef = "jboubdidadgfhjwdvikb"
$ExpectedSupabaseUrl = "https://jboubdidadgfhjwdvikb.supabase.co"
...
function Invoke-StagingSql {
    param([Parameter(Mandatory = $true)][string]$Sql)

    $Path = Join-Path ([System.IO.Path]::GetTempPath()) (
        "vyra-staging-" + [guid]::NewGuid().ToString() + ".sql"
    )
    ...
    try {
        ...
        Assert-ExitCode "Staging SQL query"
...
function Get-StagingUuid {
    param(
        [Parameter(Mandatory = $true)][string]$Sql,
        [Parameter(Mandatory = $true)][string]$Description
    )

    $Output = Invoke-StagingSql -Sql $Sql
    $Match = [regex]::Match(
        $Output,
...
if ($ProjectRef -ne $ExpectedProjectRef) {
    throw "This test is locked to staging project $ExpectedProjectRef"
}

if ($SupabaseUrl.TrimEnd("/") -ne $ExpectedSupabaseUrl) {
    throw "This test is locked to staging URL $ExpectedSupabaseUrl"
}
...
try {
    Write-Host "=== VYRA staging mock pipeline ===" -ForegroundColor Cyan
    Write-Host "Target: $SupabaseUrl"
    Write-Host "Safety: locked staging project; mock providers are reapplied now"
    ...
    Assert-ExitCode "Setting staging mock providers"
    ...
    if (-not $Health.ok -or $Health.service -ne "vyra-controller") {
        throw "Staging controller health check failed"
    }

    Invoke-StagingSql -Sql @"
do $([char]36)$([char]36)
...
"@ | Out-Null
    ...
    $ResearchJobId = Get-StagingUuid -Description "Topic Scout Research job" -Sql @"
...
"@ | Out-Null
    ...
    $ActivationBody = @{
        ...
        affiliate_url = "$ProgramUrl/referral?verified=staging"
        ...
        countries = @("US", "GR")
        verified_by = "staging-mock-pipeline"
        verification_note = "Controlled staging mock verification only"
...
"@ | Out-Null
    ...
    Write-Host "[PASS] Staging persisted the complete pipeline" -ForegroundColor Green
}
finally {
    try {
        Invoke-StagingSql -Sql @"
...
begin
  ...
  if leftovers <> 0 then
    raise exception 'Staging cleanup left % diagnostic row(s)', leftovers;
  end if;
end
$([char]36)$([char]36);
...
"@ | Out-Null
        ...
        Write-Host "[PASS] Staging diagnostic data cleaned" -ForegroundColor Green
    }
    finally {
