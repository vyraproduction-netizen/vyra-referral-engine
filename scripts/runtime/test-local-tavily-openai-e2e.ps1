...
"@ | Out-Null
	...
	Write-Host "=== VYRA controlled Tavily -> OpenAI E2E test ===" -ForegroundColor Cyan
    Write-Host "Target: $SupabaseUrl"
    Write-Host "External calls: one Tavily Advanced search; one OpenAI Content request"
    Write-Host "QA, program activation, and Publisher: local mock stages"
    Write-Host ""
...
"@
    ...
    if (
        ...
        $qaResponse.job_id -ne $qaJobId -or
        $qaResponse.qa.status -ne "approved" -or
        [decimal]$qaResponse.qa.score -lt [decimal]0.8
    ) {
        throw "QA did not approve the real OpenAI draft"
    }
    ...
    $activationBody = @{
        ...
        affiliate_url = "https://example.local/referral?run=$researchJobId"
        terms_url = "https://example.local/terms?run=$researchJobId"
        commission_type = "percentage"
        commission_value = 25
        recurring = $true
        cookie_duration_days = 30
        countries = @("EU")
        verified_by = "local-e2e-test"
        verification_note = "Controlled local mock activation"
    ...
    if (
        ...
        throw "Publisher did not claim the controlled mock job"
    }
...
"@
    ...
    if (
        $finalState -ne
        "completed|published|$expectedPublishedUrl|true"
    ) {
        throw "Mock Publisher final state is invalid: $finalState"
    }

    Write-Host "[PASS] Mock QA approved the real OpenAI draft" -ForegroundColor Green
    Write-Host "[PASS] Program activation produced a verified active link" -ForegroundColor Green
    Write-Host "[PASS] Mock Publisher completed the local publication" -ForegroundColor Green
...
where agent = 'publisher'
  and payload->>'content_id' = nullif('$contentId', '');
