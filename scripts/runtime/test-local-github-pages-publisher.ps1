...
if ($runtimeState -ne "present|mock|mock|github_pages|vyraproduction-netizen/vyra-publisher-test") {
...
if ($runtime["GITHUB_PUBLISH_BASE_URL"] -ne "https://vyraproduction-netizen.github.io/vyra-publisher-test") {
    throw "Controlled Publisher test requires the isolated GitHub Pages test site"
}
...
$expectedUrl = "https://vyraproduction-netizen.github.io/vyra-publisher-test/articles/$slug.html"
...
values (
  '$contentId'::uuid,
  'VYRA controlled GitHub Pages publication',
  '$slug',
  'en',
  'approved',
  '# Controlled VYRA Publisher Test' || E'\n\n' ||
    'This isolated diagnostic article is removed after verification.',
  'Controlled diagnostic article.',
  'VYRA Publisher Test',
  'Controlled GitHub Pages publication test.',
...
"@ | Out-Null
    ...
    Write-Host "=== VYRA controlled GitHub Pages Publisher test ===" -ForegroundColor Cyan
    Write-Host "Target: $SupabaseUrl"
    Write-Host "External operations: one GitHub Pages publication and one verified cleanup"
    ...
    if (
        ...
        $publisherResponse.provider -ne "github_pages" -or
        $publisherResponse.publication.published_url -ne $expectedUrl
    ) {
        throw "Publisher did not complete the controlled GitHub Pages job"
    }
...
"@
    ...
    if ($finalState -ne "completed|published|$expectedUrl|true|github_pages") {
        throw "GitHub Pages Publisher final state is invalid: $finalState"
    }

    Write-Host "[PASS] GitHub Pages Publisher published and verified the live diagnostic page" -ForegroundColor Green
